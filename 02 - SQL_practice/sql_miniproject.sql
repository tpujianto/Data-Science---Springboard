...
SELECT COUNT(*) AS "Free for members" FROM Facilities WHERE membercost = 0
...
SELECT `facid`, 
		...
		`monthlymaintenance` 
FROM `Facilities` 
WHERE `membercost` NOT = 0 AND `membercost` < (0.2 * `monthlymaintenance`)
...
FROM	(SELECT Facilities.name AS 'facility_name',
		 ...
		 ON Bookings.facid = Facilities.facid
		 WHERE Bookings.starttime >=  '2012-09-14'
		 AND Bookings.starttime <  '2012-09-15') AS filtered_bookings
HAVING cost > 30
ORDER BY cost DESC
...
FROM (
    SELECT f.name, 
    SUM(
        CASE WHEN b.memid =0 THEN b.slots * f.guestcost ELSE b.slots * f.membercost END ) AS total_revenue
    FROM  `Bookings` b
    INNER JOIN  `Facilities` f ON b.facid = f.facid
    GROUP BY f.name) AS selected_facilities
